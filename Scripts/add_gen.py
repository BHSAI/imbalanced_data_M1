...
gen_inactives = pd.read_csv('../Data/Input/M1_RNN_inactives.csv', index_col=0)
gen_inactives.columns = ['SMILES']
gen_inactives[['Activity']]=2

all_data_filter = pd.concat([training_set, gen_inactives]).reset_index(drop=True)
...
for i in range(inac.shape[0]):
    for j in range(gen_training.shape[0]):
        if np.array_equal(inac.Morgan2FP[i],gen_training.Morgan2FP[j]):
            print('Inactives',i,j)
            source_in.append(j)

for i in range(ac.shape[0]):
    for j in range(gen_training.shape[0]):
        if np.array_equal(ac.Morgan2FP[i],gen_training.Morgan2FP[j]):
            print('Actives',i,j)
            source_ac.append(j)

gen_training = gen_training.drop(source_in).reset_index(drop=True)
gen_training = gen_training.drop(source_ac).reset_index(drop=True)

'''
5. Combine actives, inactives and RNN/DEG, and remove duplicates with external test set
'''

final_training_set = pd.concat([ac, inac, gen_training]).reset_index(drop=True)
